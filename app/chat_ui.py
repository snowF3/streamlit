...
def _cortex_complete(prompt: str, model: str = "claude-3-5-sonnet") -> str:
    ...
    if district_name:
        ...
        try:
            pop = run_query(f"""
                ...
                WHERE m.DISTRICT_KOR_NAME = '{district_name}'
                  AND f.STANDARD_YEAR_MONTH = (SELECT MAX(STANDARD_YEAR_MONTH) FROM {SPH}.FLOATING_POPULATION_INFO)
            """)
            if not pop.empty and pop["RES"].values[0]:
                ctx_parts.append(f"[유동인구] 거주:{pop['RES'].values[0]:,.0f} 직장:{pop['WORK'].values[0]:,.0f} 방문:{pop['VISIT'].values[0]:,.0f}")
        except Exception:
            pass
        ...
        try:
            sales = run_query(f"""
                ...
                WHERE m.DISTRICT_KOR_NAME = '{district_name}' AND c.CARD_TYPE = '1'
                  AND c.STANDARD_YEAR_MONTH = (SELECT MAX(STANDARD_YEAR_MONTH) FROM {SPH}.CARD_SALES_INFO)
            """)
            if not sales.empty and sales["TOTAL"].values[0]:
                ctx_parts.append(f"[카드매출] 총:{sales['TOTAL'].values[0]/1e8:,.1f}억 커피:{sales['COFFEE'].values[0]/1e4:,.0f}만 음식:{sales['FOOD'].values[0]/1e4:,.0f}만")
        except Exception:
            pass
...
- '이거', '여기', '이 동네' 등은 현재 화면의 지역을 참조하세요.
...
def render_chat_panel(current_tab: str = "", selected_district: str = "", selected_month: str = "", page_context: str = ""):
    ...
    # ── 네이티브 Streamlit 채팅 UI ──
    # Snowflake 환경에서는 components.html이 제한적이므로 네이티브 UI 사용

    # 하단 토글
    cols = st.columns([8, 1])
    with cols[1]:
        btn_label = "💬" if not st.session_state.get("chat_open") else "✕"
        if st.button(btn_label, key="sf_chat_toggle", help="AI 에이전트"):
    ...
    <div style="background:linear-gradient(135deg,#6366F1,#8B5CF6);padding:14px 18px;border-radius:12px 12px 0 0;">
        ...
        <span style="color:rgba(255,255,255,0.6);font-size:11px;margin-left:8px;">
            {'🟢 Cortex 연결됨' if agent_online else '🔴 연결 안됨'} · Snowflake Native
        </span>
    </div>
    ...
    with chat_container:
        if not st.session_state.chat_messages:
            ...
            <div style="text-align:center;padding:50px 20px;color:#888;">
                ...
                <div style="font-size:14px;font-weight:600;color:#ccc;">동네 데이터, 무엇이든 물어보세요</div>
