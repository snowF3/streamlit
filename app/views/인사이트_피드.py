...
def render():
    ...
    with col_mid:
        ...
        same_city_all = hp[(hp["city"] == sig["city"]) & (hp["STANDARD_YEAR_MONTH"] == selected_ym) & (hp["DISTRICT_CODE"] != sig["dc"])].sort_values("hotplace_score", ascending=False)
        if not same_city_all.empty:
            for _, rel_row in same_city_all.head(5).iterrows():
                ...
                rel_hp_all = hp[(hp["DISTRICT_CODE"] == rel["dc"]) & (hp["STANDARD_YEAR_MONTH"] <= selected_ym)]
